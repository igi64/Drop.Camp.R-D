...
void DataChannelObserver::OnStateChange()
{
  ...
  QueueEvent(DataChannel::MY_STATE, static_cast<void*>(data));
  TRACE_END;
}
...
void DataChannelObserver::OnMessage(const webrtc::DataBuffer& buffer)
{
  TRACE_CALL;
	...
	QueueEvent(DataChannel::MY_MESSAGE, static_cast<void*>(data));
  TRACE_END;
}
...
void DataChannel::Run(uv_async_t* handle, int status)
{
  ...
  while(true)
  {
    ...
    if(DataChannel::MY_ERROR & evt.type)
    {
    ...
    } else if(DataChannel::MY_STATE & evt.type)
    {
    ...
    } else if(DataChannel::MY_MESSAGE & evt.type)
    {
...
NAN_SETTER(DataChannel::ReadOnly) {
  MY_INFO("PeerConnection::ReadOnly");
}
